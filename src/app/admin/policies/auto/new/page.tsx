...
function NewAutoPolicyForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const preCustomer = searchParams.get('customerId') || '';
  const [customers, setCustomers] = useState<any[]>([]);
  const [form, setForm] = useState({ customerId: preCustomer, policyStartDate: '', policyEndDate: '', premiumAmount: '', policyStatus: 'C' });
...
export default function NewAutoPolicyPage() {
  return (
    <AdminLayout title="New Auto Policy" subtitle="Create an auto insurance policy">
